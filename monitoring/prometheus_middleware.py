...
# Define metrics with service label (populated at runtime)
...
class PrometheusMiddleware(BaseHTTPMiddleware):
    """
    Middleware to collect Prometheus metrics from FastAPI apps.
    
    Usage in your FastAPI app:
        from prometheus_middleware import PrometheusMiddleware, metrics_response
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        REQUEST_LATENCY.labels(
            method=method, path=path, service=self.service_name
        ).observe(duration)
